...
export default function HomePage() {
  ...
  return (
    <CustomerLayout title="Browse Equipment" subtitle="Find and book the agricultural equipment you need">
      {/* Filters */}
      <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Search equipment..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-11" />
        </div>
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground bg-muted/50 px-4 rounded-lg h-11">
          ...
          <Button variant="ghost" size="sm" onClick={fetchData} className="h-auto p-0"><Loader2 className="w-4 h-4" /></Button>
        </div>
      </div>
        ...
        <Card className="border-dashed">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              {searchQuery || categoryFilter !== "all" ? "Try adjusting your filters or search query" : "No available equipment at the moment"}
            </p>
      ...
      <BookingModal equipment={selectedEquipment} open={bookingOpen} onOpenChange={setBookingOpen} onSuccess={fetchData} />
    </CustomerLayout>
  );
}
